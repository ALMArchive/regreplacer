...
describe("RegRepMatches", function() {
  ...
  describe("Replace All Works", function () {
    it('$v1 $v2 $v3 string replace should be word word word on matches', function () {
      regMatch = new RegRepMatches(matches);
      chai.expect(regMatch.replaceAll("word", "matches")).to.equal("word word word");
    });
  });
  describe("Replace All Works with Function", function () {
    it('$v1 $v2 $v3 string replace should be This is cool on matches', function () {
      regMatch = new RegRepMatches(matches);
      chai.expect(regMatch.replaceAll(e => e.toUpperCase(), "matches")).to.equal("$V1 $V2 $V3");
    });
    it('$v1 $v2 $v3 string replace should be This is cool on matches', function () {
      regMatch = new RegRepMatches(matches);
      chai.expect(regMatch.replaceAll(() => "?", "matches")).to.equal("? ? ?");
    });
  });
});
